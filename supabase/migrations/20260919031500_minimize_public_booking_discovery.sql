-- Public booking discovery must expose availability, not internal blackout notes.
create or replace function public.get_public_blocked_dates_v3(p_booking_slug text)
returns table(blocked_date date) language sql stable security definer set search_path='' as $$
 ...
 and b.blocked_date >= current_date
 order by b.blocked_date
$$;
revoke all on function public.get_public_blocked_dates_v3(text) from public,anon,authenticated;
grant execute on function public.get_public_blocked_dates_v3(text) to anon,authenticated,service_role;
revoke all on function public.get_public_blocked_dates_v2(text) from public,anon,authenticated;

-- Public profile: remove owner user UUID and raw Stripe/account implementation signal.
create or replace function public.get_public_booking_profile_v3(booking_slug_param text)
returns table(business_name text,logo_url text,phone text,email text,opening_time time,closing_time time,working_days text[],currency text,
payments_enabled boolean,service_radius_miles numeric,service_address text,service_coordinates jsonb,buffer_time_before integer,buffer_time_after integer,
...
language sql stable security definer set search_path='' as $$
 select w.name,ws.logo_url,ws.phone,ws.email::text,ws.opening_time,ws.closing_time,ws.working_days,trim(w.currency_code),
...
revoke all on function public.get_public_booking_profile_v2(text) from public,anon,authenticated;
