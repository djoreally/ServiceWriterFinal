...
alter table public.in_app_notification_push_outbox
...
begin
  ...
  set status = case
      ...
      when attempts >= 8 then 'discarded'
      else 'failed'
    end,
    ...
    next_attempt_at = case when p_sent then next_attempt_at else timezone('utc', now()) + make_interval(secs => greatest(30, least(coalesce(p_retry_seconds, 300), 86400))) end,
    locked_at = null,
    worker_id = null,
    updated_at = timezone('utc', now())
  where id = p_id
...
revoke all on function public.claim_in_app_push_outbox(integer, text) from public, anon, authenticated;
revoke all on function public.complete_in_app_push_outbox(uuid, text, boolean, text, integer) from public, anon, authenticated;
