create or replace function public.complete_appointment_closeout_v1(p_workspace_id uuid, p_appointment_id uuid)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  update public.service_records
     set subtotal=v_subtotal,
         ...
         currency_code='USD',
         metadata=coalesce(metadata,'{}'::jsonb)||jsonb_build_object('financial_source','appointment_closeout','appointment_item_count',v_item_count),
         updated_at=now()
  ...
  if v_invoice_id is null then
    insert into public.invoices(
      workspace_id, customer_id, vehicle_id, status,
      ...
      v_subtotal, v_tax, v_total, 0, now(), v_actor,
      jsonb_build_object(
    ...
    ) returning id, invoice_number, status into v_invoice_id, v_invoice_number, v_invoice_status;
    ...
    if not found then
      insert into public.invoice_lines(
        ...
        1, v_subtotal, 0, 0,
        jsonb_build_object('source','appointment_completion_fallback')
      );
    end if;
  end if;
  ...
  select id, status into v_payment_id, v_payment_status
   ...
   where workspace_id=p_workspace_id
     and metadata->>'appointment_id'=p_appointment_id::text
     and status in ('pending'::public.payment_status,'succeeded'::public.payment_status,'partially_refunded'::public.payment_status)
   order by created_at asc
   limit 1
   for update;
  ...
  if v_payment_id is null then
    insert into public.payments(
      ...
      jsonb_build_object(
        ...
        'subtotal',v_subtotal,
        'tax_amount',v_tax,
        'subtotal_cents',round(v_subtotal*100),
        'tax_amount_cents',round(v_tax*100)
      )
