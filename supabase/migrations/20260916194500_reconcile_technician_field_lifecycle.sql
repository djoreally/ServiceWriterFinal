-- Follow-up: align canonical technician transitions to the real field lifecycle.
-- Adds could_not_complete and removes obsolete pause/delay behavior.

alter table public.dispatch_events drop constraint if exists dispatch_events_event_type_check;
alter table public.dispatch_events add constraint dispatch_events_event_type_check
  check (event_type = any (array[
    'assigned','reassigned','assignment_confirmed','unassigned','status_changed',
    'en_route','arrived','started','completed','cancelled','note','could_not_complete'
  ]::text[]));
...
begin
  ...
  if v_source='appointment' then
    ...
    elsif v_next = 'could_not_complete' then
      if v_current_status <> 'in_progress'
        then raise exception 'INVALID_TRANSITION: Only an in-progress appointment can be marked could not complete'; end if;
      update public.appointments set
        metadata=jsonb_set(coalesce(metadata,'{}'::jsonb),'{dispatch_status}',to_jsonb('could_not_complete'::text),true),
        updated_at=now()
      where id=p_job_id;
      v_event_type:='could_not_complete'; v_next:='could_not_complete';
    ...
    if v_next in ('en_route','arrived') then
      ...
      if v_current_status <> 'in_progress'
        then raise exception 'INVALID_TRANSITION: Only an in-progress work order can be marked could not complete'; end if;
