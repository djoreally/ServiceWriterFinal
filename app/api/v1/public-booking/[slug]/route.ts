...
async function profileForSlug(supabase: Awaited<ReturnType<typeof createSupabaseServerClient>>, slug: string) {
  const { data, error } = await (supabase as any).rpc("get_public_booking_profile_v2", { booking_slug_param: slug });
...
export async function GET(request: Request, context: { params: Promise<{ slug: string }> }) {
  try {
    ...
    if (query.section === "catalog") {
      ...
      const v1 = await (supabase as any).rpc("get_public_service_catalog", { business_user_id: businessUserId });
    ...
    if (query.section === "packages") {
      const { data, error } = await (supabase as any).rpc("get_public_service_packages", { business_user_id: businessUserId });
    ...
    if (query.section === "slots") {
      ...
      const { data, error } = await (supabase as any).rpc("get_booked_slots", { business_user_id: businessUserId, booking_date: query.date });
      if (error || !Array.isArray(data)) throw unavailable();
      return json({ data }, { headers: { "Cache-Control": "no-store" } });
    }

    if (query.section === "blocked_dates") {
      const { data, error } = await (supabase as any).rpc("get_public_blocked_dates", {
        p_business_user_id: businessUserId,
    ...
    const { data, error } = await (supabase as any).rpc("get_public_booking_settings", { p_business_user_id: businessUserId });
