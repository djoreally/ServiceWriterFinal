...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const closeoutData = object(closeout); const serviceRecordId = String(closeoutData.service_record_id ?? ""); const invoiceId = String(closeoutData.invoice_id ?? ""); const paymentId = String(closeoutData.payment_id ?? "");
