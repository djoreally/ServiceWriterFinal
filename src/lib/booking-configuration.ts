...
export interface BookingVehicleConfiguration {
  ...
  oil?: { engine?: string; oilType?: string; oilCapacity?: string; oilFilter?: string; capacitySource?: "db" | "ai" | "manual" };
  tire?: {
    frontSize: string; rearSize?: string; source: "oe" | "manual"; frontQuantity: number; rearQuantity: number;
    inventoryItemId?: string; sku?: string; productName?: string; unitPrice?: number;
...
type SelectedService = { id: string; name: string; default_price: number; booking_requirements?: BookingRequirement[] };
type SelectedPackage = { id: string; name: string; package_price: number; services: Array<{ id: string; name: string; price: number; quantity: number; booking_requirements?: BookingRequirement[] }> };
type VehicleSelection = { services: SelectedService[]; package: SelectedPackage | null };

function inferRequirements(selection: VehicleSelection | undefined): Set<BookingRequirement> {
  const requirements = new Set<BookingRequirement>();
  const selected = selection?.package?.services ?? selection?.services ?? [];
  for (const service of selected) {
    for (const requirement of service.booking_requirements ?? []) requirements.add(requirement);
    const name = service.name.toLowerCase();
    if (/\b(oil|lube|lubrication|fluid service)\b/.test(name)) requirements.add("oil_fitment");
    if (/\b(tire|tires|tyre|wheel|rotation|tpms)\b/.test(name)) { requirements.add("tire_fitment"); requirements.add("tire_quantity"); }
    if (/\b(detail|detailing|wash|ceramic|coating|wax|polish|interior|exterior)\b/.test(name)) requirements.add("detailing_assessment");
  }
  return requirements;
}
...
function buildTireConfiguration(vehicle: VehicleData): NonNullable<BookingVehicleConfiguration["tire"]> {
  ...
  if (vehicle.tireInventoryItemId && !hasExplicitQuantity) throw new Error("Tire quantity must be explicitly selected before reserving inventory");
  return {
    ...
    options: { mountAndBalance: vehicle.tireMountAndBalance ?? true, tpms: vehicle.tireTpms ?? false, disposal: vehicle.tireDisposal ?? true },
  };
}

export function buildAppointmentBookingConfiguration(vehicles: VehicleData[], vehicleServiceSelections: Record<string, VehicleSelection> = {}): AppointmentBookingConfiguration {
  return {
    schemaVersion: BOOKING_CONFIGURATION_VERSION,
    capturedAt: new Date().toISOString(),
    vehicles: vehicles.filter((v) => v.year && v.make && v.model).map((v) => {
      const selection = vehicleServiceSelections[v.id];
      const requirements = inferRequirements(selection);
      const includeOil = requirements.has("oil_fitment");
      const includeTire = requirements.has("tire_fitment");
      const includeDetailing = requirements.has("detailing_assessment");
      const additional = v.additionalSpecs ?? {};
      const oilFilter = typeof additional.oil_filter === "string" ? additional.oil_filter : undefined;
      return {
...
export function configurationInventoryTotal(configuration: AppointmentBookingConfiguration): number {
  return configuration.vehicles.reduce((sum, vehicle) => vehicle.tire?.unitPrice ? sum + vehicle.tire.unitPrice * (vehicle.tire.frontQuantity + vehicle.tire.rearQuantity) : sum, 0);
}
