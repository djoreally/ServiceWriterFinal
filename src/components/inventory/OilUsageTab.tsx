...
import { OilUsageReconcileButton } from "@/components/inventory/OilUsageReconcileButton";
import { toast } from "@/components/ui/sonner";
import { fetchOilUsage, type FetchOilUsageResult, type UsageRow } from "@/application/queries/inventory-usage.query";

const ALL = "__all__";
function sourceLabel(source: UsageRow["source"]) { return source === "inventory_reconciled" ? "Inventory reconciled" : "Completed service"; }

function downloadCsv(filename: string, rows: UsageRow[]) {
  const header = ["Date", "Customer", "Vehicle", "Oil", "Quarts", "Source", "Appointment ID"];
  const esc = (value: unknown) => { const s = value == null ? "" : String(value); return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s; };
  const body = rows.map((row) => [format(new Date(row.consumed_at), "M/d/yyyy h:mm a"), row.customer_name, row.vehicle_label, row.item_name, row.qty_in_qts.toFixed(2), sourceLabel(row.source), row.appointment_id].map(esc).join(","));
...
export function OilUsageTab() {
  ...
  const loadUsage = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setData(await fetchOilUsage({ from, to, itemIds: itemId !== ALL ? [itemId] : undefined, source: source === "completed_service" || source === "inventory_reconciled" ? source : null, search: debouncedSearch || null }));
    } catch (error: unknown) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div><h3 className="text-xl font-bold">Oil Usage</h3><p className="text-sm text-muted-foreground">Derived from oil quantity recorded on completed services. Inventory setup is optional and never required for this history.</p></div>
      <div className="flex flex-col gap-2 sm:flex-row"><DateRangeFilter dateRange={dateRange} onDateRangeChange={setDateRange} preset={preset} onPresetChange={(next) => { setPreset(next); if (next !== "custom") setDateRange(getDateRangeFromPreset(next)); }} /><Button variant="outline" className="gap-2" disabled={!data?.rows.length} onClick={() => data && downloadCsv(`oil-usage-${format(new Date(), "yyyy-MM-dd")}.csv`, data.rows)}><Download className="h-4 w-4" /> Export CSV</Button></div>
    </div>

    <div className="flex flex-wrap items-center gap-2">
      <div className="relative min-w-[220px] flex-1 max-w-sm"><Search className="absolute left-2 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search customer, vehicle, oil…" className="pl-8" /></div>
      <Select value={itemId} onValueChange={setItemId}><SelectTrigger className="w-[190px]"><SelectValue placeholder="Oil type" /></SelectTrigger><SelectContent><SelectItem value={ALL}>All oil types</SelectItem>{data?.availableItems.map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</SelectContent></Select>
      <Select value={source} onValueChange={setSource}><SelectTrigger className="w-[190px]"><SelectValue placeholder="Reconciliation" /></SelectTrigger><SelectContent><SelectItem value={ALL}>All completed usage</SelectItem><SelectItem value="completed_service">Service only</SelectItem><SelectItem value="inventory_reconciled">Inventory reconciled</SelectItem></SelectContent></Select>
    ...
    <Card><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>WHEN</TableHead><TableHead>CUSTOMER</TableHead><TableHead>VEHICLE</TableHead><TableHead>OIL</TableHead><TableHead className="text-right">QTY</TableHead><TableHead>STATUS</TableHead></TableRow></TableHeader><TableBody>{!data?.rows.length ? <TableRow><TableCell colSpan={6} className="py-12 text-center text-muted-foreground">{loading ? "Loading…" : "No completed jobs with oil entered in this range"}</TableCell></TableRow> : data.rows.map((row) => <TableRow key={row.id}><TableCell className="text-sm">{format(new Date(row.consumed_at), "M/d/yyyy h:mm a")}</TableCell><TableCell>{row.customer_name ?? "—"}</TableCell><TableCell>{row.vehicle_label ?? "—"}</TableCell><TableCell className="font-medium">{row.item_name}</TableCell><TableCell className="text-right">{row.qty_in_qts.toFixed(2)} qt</TableCell><TableCell><div className="flex items-center gap-2"><Badge variant={row.source === "inventory_reconciled" ? "outline" : "secondary"}>{sourceLabel(row.source)}</Badge>{row.source === "completed_service" && <OilUsageReconcileButton serviceRecordId={row.id} oilType={row.item_name} quarts={row.qty_in_qts} onReconciled={() => void loadUsage()} />}</div></TableCell></TableRow>)}</TableBody></Table></div></CardContent></Card>
