...
import {
  fetchAppointmentInspectionGate,
  type AppointmentInspectionGate,
} from "@/application/queries/inspections.query";
...
export function JobActionButton({ appointment, onUpdated, className }: JobActionButtonProps) {
  ...
  const refreshGate = useCallback(async () => {
    try {
      const g = await fetchAppointmentInspectionGate(appointment.id);
      setGate(g);
    } catch {
      setGate({ required: [], pendingCount: 0 });
    }
  }, [appointment.id]);
  ...
  const handleStart = async () => {
    ...
    onUpdated();
    // If there's no inspection, the next render will show "Complete Job".
  };
  ...
  if (step === "loading") {
    return (
      <Button className={className} variant="default" disabled>
  ...
  if (step === "start") {
    return (
      <Button className={className} variant="default" onClick={handleStart} disabled={starting}>
    ...
    return (
      <>
        ...
        >
          ...
          Fill Inspection ({pending.length} pending)
        </Button>
        <Dialog open={showInspection} onOpenChange={setShowInspection}>
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Required Inspection</DialogTitle>
              <DialogDescription>
                Complete the inspection below to unlock job completion.
              </DialogDescription>
            </DialogHeader>
            <InspectionPerformer
  ...
  return (
    <>
      <Button className={className} variant="default" onClick={() => setShowComplete(true)}>
