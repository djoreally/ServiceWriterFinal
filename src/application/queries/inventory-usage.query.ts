...
export async function fetchOilUsage(params:FetchOilUsageParams):Promise<FetchOilUsageResult>{
  ...
  const servicesRes=await db.from("service_records").select("id,appointment_id,customer_id,vehicle_id,completed_at,oil_quarts_used,metadata").eq("workspace_id",workspaceId).eq("status","completed").gt("oil_quarts_used",0).gte("completed_at",params.from.toISOString()).lte("completed_at",params.to.toISOString()).order("completed_at",{ascending:false}).limit(5000);
  if(servicesRes.error)throw new Error(servicesRes.error.message);
  const services=(servicesRes.data??[]) as Array<{id:string;appointment_id:string|null;customer_id:string|null;vehicle_id:string|null;completed_at:string|null;oil_quarts_used:number|string|null;metadata:unknown}>; if(!services.length)return empty;
  const customerIds=[...new Set(services.map(r=>r.customer_id).filter((v):v is string=>!!v))], vehicleIds=[...new Set(services.map(r=>r.vehicle_id).filter((v):v is string=>!!v))], appointmentIds=[...new Set(services.map(r=>r.appointment_id).filter((v):v is string=>!!v))], serviceIds=services.map(r=>r.id);
  const [customersRes,vehiclesRes,specsRes,appointmentsRes,movementsRes]=await Promise.all([
    customerIds.length?db.from("customers").select("id,first_name,last_name,company_name").eq("workspace_id",workspaceId).in("id",customerIds):Promise.resolve({data:[],error:null}),
    vehicleIds.length?db.from("vehicles").select("id,year,make,model").eq("workspace_id",workspaceId).in("id",vehicleIds):Promise.resolve({data:[],error:null}),
    vehicleIds.length?db.from("vehicle_service_specs").select("vehicle_id,oil_type").eq("workspace_id",workspaceId).in("vehicle_id",vehicleIds):Promise.resolve({data:[],error:null}),
    appointmentIds.length?db.from("appointments").select("id,metadata").eq("workspace_id",workspaceId).in("id",appointmentIds):Promise.resolve({data:[],error:null}),
  ...
  const customerMap=new Map((customersRes.data??[]).map((r:any)=>[r.id,r])), vehicleMap=new Map((vehiclesRes.data??[]).map((r:any)=>[r.id,r])), specMap=new Map((specsRes.data??[]).map((r:any)=>[r.vehicle_id,r.oil_type as string|null])), appointmentMap=new Map((appointmentsRes.data??[]).map((r:any)=>[r.id,r.metadata])), reconciled=new Set((movementsRes.data??[]).map((r:any)=>r.service_record_id as string));
