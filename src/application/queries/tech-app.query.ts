...
export async function fetchTechJobDetailBundle(jobId: string) {
  ...
  if (fleetResult.data) {
    ...
    const mappedJob = {
      ...
      user_id: f.user_id,
      customers: { name: f.fleet_clients?.company_name, phone: f.fleet_clients?.phone },
