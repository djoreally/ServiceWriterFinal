...
export async function fetchInventoryOverview(): Promise<InventoryOverviewResult> {
  ...
  const items = ((itemsRes.data ?? []) as Omit<InventoryItem, "quantity">[]).map((item) => ({
    ...item,
  ...
  const vanInventory: VanInventoryLink[] = stock
    .filter((row) => nonWarehouseIds.has(row.location_id))
    .map((row) => ({ inventory_item_id: row.inventory_item_id, van_id: row.location_id, quantity: Number(row.quantity ?? 0) }));

  const reservations: ReservationLink[] = ((reservationsRes.data ?? []) as Array<{
    inventory_item_id: string;
    ...
    source: row.location_id && nonWarehouseIds.has(row.location_id) ? "van" : "warehouse",
