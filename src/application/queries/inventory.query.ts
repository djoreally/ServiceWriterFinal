 ...
 *
 * Oil usage is intentionally not sourced from these tables. Completed service
 * records remain authoritative for actual oil consumption.
 */
...
/** Compatibility name retained for the existing Inventory UI. */
export interface Van { id: string; name: string }
/** Compatibility shape: van_id is now an inventory location id. */
...
export async function fetchInventoryOverview(): Promise<InventoryOverviewResult> {
  ...
  const [itemsRes, locationsRes, stockRes, reservationsRes] = await Promise.all([
    supabase
      ...
      .eq("is_active", true)
      .order("name"),
    supabase.from("inventory_locations").select("id,name,location_type").eq("workspace_id", workspaceId).eq("is_active", true).order("name"),
    supabase.from("inventory_stock").select("inventory_item_id,location_id,quantity").eq("workspace_id", workspaceId),
    supabase.from("inventory_reservations").select("inventory_item_id,quantity,location_id,status").eq("workspace_id", workspaceId).eq("status", "reserved"),
  ]);

  for (const result of [itemsRes, locationsRes, stockRes, reservationsRes]) {
    if (result.error) throw new Error(result.error.message || "Failed to fetch inventory");
  }
  ...
  const reservations: ReservationLink[] = ((reservationsRes.data ?? []) as Array<{ inventory_item_id: string; quantity: number | string; location_id: string | null; status: string }>).map((row) => ({
