...
export async function fetchAppointmentWithRelations(id: string, _userId: string) {
  ...
  try {
    ...
    return { data: {
      ...
      scheduled_date: start.date, scheduled_time: start.time, duration_minutes: durationMinutes,
      customer, vehicle, service_catalog: serviceCatalog,
      ...
      estimated_cost: Number(metadata.estimated_cost ?? 0), tax_amount: Number(metadata.tax_amount ?? 0),
      location_address: text(metadata.location_address), customer_city: text(metadata.customer_city), customer_state: text(metadata.customer_state), customer_postal_code: text(metadata.customer_postal_code),
      intake_responses: metadata, assigned_technician_id: row.assigned_user_id ?? null,
      actual_start_time: text(metadata.actual_start_time),
    }, error: null };
