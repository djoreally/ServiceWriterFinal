...
export async function updateUpsell(id: string, payload: { name: string; description: string | null; default_price: number; is_active: boolean }): Promise<void> {
  const { workspaceId } = await requireContext();
  const { error } = await db.from("service_catalog").update({ name: payload.name, description: payload.description, labor_price: payload.default_price, is_active: payload.is_active }).eq("workspace_id", workspaceId).eq("id", id);
  if (error) throw error;
}
export async function toggleUpsellActive(id: string, currentlyActive: boolean): Promise<void> {
  const { workspaceId } = await requireContext();
  const { error } = await db.from("service_catalog").update({ is_active: !currentlyActive }).eq("workspace_id", workspaceId).eq("id", id);
  if (error) throw error;
}
export async function loadDefaultUpsellTemplates(existingNames: Set<string>): Promise<number> {
  ...
  const { error } = await db.from("service_catalog").insert(toInsert.map((item, index) => ({
    workspace_id: workspaceId, name: item.name, description: item.description, category: "Add-ons", estimated_minutes: 15,
    labor_price: item.price, is_active: true, metadata: { is_upsell: true, sort_order: 900 + index, pricing_mode: "flat" },
  })));
  if (error) throw error;
...
export async function addUpsell(payload: { name: string; description: string | null; default_price: number }): Promise<void> {
  const { workspaceId } = await requireContext();
  const { error } = await db.from("service_catalog").insert({ workspace_id: workspaceId, name: payload.name, description: payload.description, category: "Add-ons", estimated_minutes: 15, labor_price: payload.default_price, is_active: true, metadata: { is_upsell: true, pricing_mode: "flat" } });
  if (error) throw error;
}
