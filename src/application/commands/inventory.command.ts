...
export async function createInventoryItem(payload: InventoryItemWritePayload): Promise<void> {
  ...
  const { data, error } = await (supabase as any)
    ...
    .single();
  if (error) throw new Error(error.message);
  const stockResult = await (supabase as any).rpc("set_inventory_item_stock", { p_item_id: data.id, p_quantity: quantity, p_reason: "initial stock" });
...
export async function updateInventoryItem(id: string, payload: InventoryItemWritePayload): Promise<void> {
  ...
  const { error } = await (supabase as any)
    ...
    .eq("is_active", true);
  if (error) throw new Error(error.message);
  const stockResult = await (supabase as any).rpc("set_inventory_item_stock", { p_item_id: id, p_quantity: quantity, p_reason: "inventory item edit" });
  if (stockResult.error) throw new Error(stockResult.error.message);
}
