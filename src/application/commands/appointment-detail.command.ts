...
export async function updateAppointmentStatus(id: string, status: string) {
  ...
  try {
    const { data: prev } = await supabase.from("appointments").select("status,workspace_id").eq("workspace_id", context.workspaceId).eq("id", id).maybeSingle();
    fromStatus = prev?.status ?? undefined;
    if (prev?.workspace_id) {
      queueMicrotask(() => trackAppointmentStatusChanged({
        appointment_id: id,
        organization_id: prev.workspace_id,
...
export async function deleteAppointment(id: string) {
  ...
  return supabase.from("appointments").delete().eq("workspace_id", context.workspaceId).eq("id", id);
}

/**
 * Start the job through the authenticated Supabase browser session. The old
 * Next API PATCH path dropped the incoming Request when validating membership,
 * causing a false "Authentication required" response even for signed-in users.
 */
export async function startAppointmentJob(appointmentId: string): Promise<{ success: boolean; alreadyStarted?: boolean; error?: string }> {
  try {
    ...
    const { data, error } = await supabase
      ...
      .update({
        status: "in_progress",
        metadata: { ...metadata, actual_start_time: new Date().toISOString() },
