...
| Next.js server-only integrations | `SUPABASE_SERVICE_ROLE_KEY`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `RESEND_WEBHOOK_SIGNING_SECRET`, `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_FROM_NUMBER` | Next.js Production environment only | Store as encrypted Vercel secrets. Never use these in Vite variables, client components, browser bundles, or `NEXT_PUBLIC_` names. |
